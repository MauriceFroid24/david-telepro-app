...
st.markdown(
    """
    <style>
    .block-container {padding-top: 0.8rem; padding-bottom: 1.2rem; max-width: 760px;}
    h1 {font-size: 1.55rem; margin-bottom: 0.1rem;}
    h2, h3 {margin-top: 0.5rem;}
    .script {
        background:#f2f4f7; border-left:5px solid #344054; border-radius:10px;
        padding:12px 14px; margin:10px 0 16px 0; line-height:1.45;
    }
    .ok {background:#ecfdf3; border:1px solid #abefc6; color:#05603a; border-radius:10px; padding:10px; margin:8px 0;}
    .warn {background:#fff7e6; border:1px solid #ffd591; color:#7a4b00; border-radius:10px; padding:10px; margin:8px 0;}
    .bad {background:#fff1f0; border:1px solid #ffa39e; color:#8c1d18; border-radius:10px; padding:10px; margin:8px 0;}
    .mini {font-size:.88rem; color:#667085;}
    div[data-testid="stMetric"] {border:1px solid #e4e7ec; border-radius:10px; padding:8px;}
    .stButton > button {width:100%; height:2.8rem; border-radius:10px;}
    .stDownloadButton > button {width:100%; height:2.8rem; border-radius:10px;}
    </style>
    """,
...
PAGES = [
    "Contact", "Projet", "Logement", "Motivation", "Engagement", "Décideurs & RDV", "Documents", "Rapport"
]
...
def missing_items():
    checks = [
        ...
        ("Surface", st.session_state.surface), ("Chauffage actuel", st.session_state.chauffage),
        ("Âge système", st.session_state.age_chaudiere), ("Motivation", st.session_state.declencheur),
        ("Gêne principale", st.session_state.gene), ("Engagement", st.session_state.pret_lancer),
        ("Décideurs", st.session_state.decideurs), ("Tous décideurs présents", st.session_state.tous_presents),
        ("Adresse complète", st.session_state.adresse), ("Date RDV", st.session_state.date_rdv),
        ("Heure RDV", st.session_state.heure_rdv), ("Documents", st.session_state.docs_prets),
...
st.caption("Version iPad éco batterie — page par page, rapide, sans surcharge visuelle.")
st.progress((page + 1) / len(PAGES))

h1, h2, h3 = st.columns(3)
h1.metric("Page", f"{page+1}/{len(PAGES)}")
...
if page == 0:
    ...
    script("Avant de parler de rendez-vous, je veux comprendre ce que vous souhaitez améliorer : chauffage, confort d’été, isolation, facture ou production d’énergie.")
    st.multiselect(
        "Projet identifié",
    ...
    if st.session_state.zone in ["H2", "H3"]:
        box("Zone H2/H3 : ne pas annoncer les conditions H1. L’expert vérifiera les aides exactes.", "warn")
    ...
    script("Je vais vous poser quelques questions rapides. L’objectif est d’éviter de déplacer un expert si le logement ne correspond pas du tout aux critères techniques.")
    ...
    script("Pour que l’intervention soit utile, j’ai besoin de comprendre ce qui vous a poussé à faire la demande. L’expert ne se déplace que si le projet peut avoir un vrai intérêt pour vous.")
